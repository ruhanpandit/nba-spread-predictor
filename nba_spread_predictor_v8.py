...
def _input_cols(df):
    """
    Only keep engineered features (rolling stats, differentials, flags).
    Explicitly exclude raw per-game NBA API columns that only exist in training
    matchups but not at inference time.
    """
    # Raw NBA API stat columns that come from the games dataframe itself
    RAW_STATS = {
        "MIN","PTS","FGM","FGA","FG_PCT","FG3M","FG3A","FG3_PCT",
        "FTM","FTA","FT_PCT","OREB","DREB","REB","AST","STL","BLK",
        "TOV","PF","PLUS_MINUS","VIDEO_AVAILABLE","TEAM_ABBREVIATION",
        "TEAM_NAME","GAME_DATE","SEASON_ID","MATCHUP","WL","SEASON",
    }
    bad = {
    ...
    # Also exclude H_/A_ prefixed versions of raw stats
    bad |= {f"H_{s}" for s in RAW_STATS} | {f"A_{s}" for s in RAW_STATS}

    return [c for c in df.columns
            if c not in bad
            and "PTS_ACTUAL" not in c
            and "TEAM_NAME" not in c
...
class Model:
    ...
    def predict_row(self, row_df):
        ...
        # Scale confidence honestly: 5% improvement over baseline → ~40%, 15% → ~75%
        raw_imp = max(0.0, 1 - self.cv_mae/self.base_mae)
        c = round(min(0.85, raw_imp * 8), 2)
...
def per_game_confidence(edge, mae, home_abbr, away_abbr, feat_df):
    """
    Compute a per-game confidence score (0-100) based on:
      1. Edge size relative to MAE  — bigger edge vs uncertainty = more confident
      2. Team consistency           — low scoring std dev = more predictable team
      3. Sample size                — more past games = more reliable features
    """
    # Factor 1: edge/MAE ratio — how many "error bars" does the edge span?
    # edge of 1x MAE → weak, 2x → moderate, 3x+ → strong
    if mae <= 0:
        edge_score = 0.5
    else:
        ratio = abs(edge) / mae
        edge_score = min(1.0, ratio / 3.0)   # caps at 1.0 when edge >= 3x MAE

    # Factor 2: team consistency (lower PTS std dev = more predictable)
    consistency_scores = []
    for abbr in [home_abbr, away_abbr]:
        ...
        if tid:
            ...
            if not rows.empty:
                last = rows.iloc[-1]
                pts_std = last.get("R10_PTS_mu", np.nan)  # using R10 as proxy
                # Lower std across recent games = more consistent team
                std_col = "R10_AST_sd"  # AST std as consistency proxy
                std_val = last.get(std_col, 3.0) if std_col in last.index else 3.0
                if pd.notna(std_val) and std_val > 0:
                    # std of ~2 = very consistent, ~5 = volatile
                    consistency_scores.append(min(1.0, 2.0 / max(std_val, 0.5)))
    consistency = float(np.mean(consistency_scores)) if consistency_scores else 0.5

    # Factor 3: sample size (how many games does each team have in feat_df?)
    sample_scores = []
    for abbr in [home_abbr, away_abbr]:
        tid = _tid(abbr)
        if tid:
            n = len(feat_df[feat_df["TEAM_ID"] == int(tid)])
            # 20+ games = full confidence, fewer = less reliable
            sample_scores.append(min(1.0, n / 20.0))
    sample = float(np.mean(sample_scores)) if sample_scores else 0.5

    # Weighted blend: edge size matters most
    raw = 0.60 * edge_score + 0.25 * consistency + 0.15 * sample

    # Scale to a readable 20-75% range (honest — no model should claim 90%+ on NBA spreads)
    scaled = 0.20 + raw * 0.55
...
def make_pick(game, model, feat_df):
    ...
    if spread is not None:
        # ESPN spread: e.g. spread_fav="BOS", spread=-8.5 means BOS -8.5 (home favored 8.5)
        # Convert to home_line: if spread_fav is home team, home_line = spread (negative = home favored)
        # margin > 0 means home won by that amount
        stm = game.get("spread_fav","")
        if stm and stm.upper() == game["home"].upper():
            home_line = spread          # e.g. -8.5 means home must win by 8.5
        elif stm and stm.upper() == game["away"].upper():
            home_line = -spread         # e.g. away -8.5 → home +8.5
        else:
            home_line = spread          # fallback

        # edge > 0: model thinks home covers; edge < 0: model thinks away covers
        edge = margin - home_line
        rec["edge"] = round(edge, 1)
        rec["home_line"] = round(home_line, 1)

        # Per-game confidence based on edge size, team consistency, sample size
        rec["conf"] = per_game_confidence(edge, res["mae"], game["home"], game["away"], feat_df)

        if abs(edge) < 2:
            rec["pick"],rec["strength"] = "PASS — edge too small","WEAK"
        elif edge > 0:
            rec["pick"]     = f"{game['home']} covers {home_line:+.1f}"
            rec["strength"] = "STRONG" if abs(edge) > 5 else "MODERATE"
        else:
            rec["pick"]     = f"{game['away']} covers {-home_line:+.1f}"
            rec["strength"] = "STRONG" if abs(edge) > 5 else "MODERATE"
    else:
        rec["conf"] = 0.20
        rec["pick"],rec["strength"],rec["edge"],rec["home_line"] = f"No line — proj: {margin:+.1f}","N/A",None,None
    return rec
...
def pprint(r):
    ...
    if r["type"]=="live":
        ...
        if r.get("spread") is not None:
            hl = r.get("home_line", r["spread"])
            print(f"    Vegas line (home): {hl:+.1f}  |  Model edge: {r['edge']:+.1f}")
        print(f"    📌 PICK: {r['pick']}  [{r['strength']}]")
        print(f"    Confidence: {_conf_bar(r['conf'])}")
